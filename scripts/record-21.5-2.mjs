#!/usr/bin/env node
/**
 * arac-ici-inis.html (500T) → export/21.5-2.mp4
 * Kullanım: npm run record:21.5-2
 * Önce: npm start
 */
...
const URL = 'http://127.0.0.1:3000/arac-ici-inis.html';
/** 10 durak × 5 sn + son durak 7 sn + pay */
const RECORD_SEC = Number(process.env.RECORD_SEC || 65);
...
async function main() {
  ...
  const mp4Path = path.join(EXPORT_DIR, '21.5-2.mp4');
