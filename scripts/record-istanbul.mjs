#!/usr/bin/env node
/**
 * istanbul.html (34E) animasyonunu MP4 olarak kaydeder.
 * Kullanım: npm run record:istanbul
 * Önce: npm start
 */
...
const URL = 'http://127.0.0.1:3000/istanbul.html';
/** 6 durak × 10 sn + pay */
const RECORD_SEC = Number(process.env.RECORD_SEC || 62);
...
async function main() {
  ...
  const mp4Path = path.join(EXPORT_DIR, 'istanbul.mp4');
