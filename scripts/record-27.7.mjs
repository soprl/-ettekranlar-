#!/usr/bin/env node
/**
 * 27.7.html (30M) animasyonunu MP4 olarak kaydeder.
 * Kullanım: npm run record:27.7
 * Önce: npm start
 */
...
const URL = 'http://127.0.0.1:3000/27.7.html';
/** 10 durak tam tur (~65 sn) + pay */
const RECORD_SEC = Number(process.env.RECORD_SEC || 75);
...
async function main() {
  ...
  console.log(`Kayıt: ${RECORD_SEC}s · 1920×198 · ${pageUrl}`);
  const browser = await chromium.launch({ headless: true });
  const context = await browser.newContext({
    viewport: { width: 1920, height: 198 },
    deviceScaleFactor: 1,
    recordVideo: {
      dir: EXPORT_DIR,
      size: { width: 1920, height: 198 },
    },
  });
  ...
  const mp4Path = path.join(EXPORT_DIR, '27.7.mp4');
