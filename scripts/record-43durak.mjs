#!/usr/bin/env node
/**
 * 43durak.html (akıllı durak) animasyonunu MP4 olarak kaydeder.
 * Kullanım: npm run record:43durak
 * Önce: npm start
 */
...
const URL = 'http://127.0.0.1:3000/43durak.html?record=1';
/** Hava döngüsü + birkaç otobüs animasyonu */
const RECORD_SEC = Number(process.env.RECORD_SEC || 80);
...
async function main() {
  ...
  const mp4Path = path.join(EXPORT_DIR, '43durak.mp4');
